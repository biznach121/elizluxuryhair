...
export async function generateStaticParams() {
  const r = await getStoreServerClient().catalogue.getProducts({ limit: 10_000 });
  ...
  return (
    <>
      ...
      <nav
        ...
        className="max-w-7xl mx-auto px-6 sm:px-8 pt-6 text-[12px] font-mono text-muted-foreground flex flex-wrap items-center gap-x-2 gap-y-1 min-w-0"
      >
        ...
        <span>/</span>
        <span className="text-foreground/90 truncate min-w-0 max-w-full">{data.product.name}</span>
      </nav>
