...
// Accepted payment methods (Ghana-first). Presentational, like the social ICONS.
const PAYMENTS = ["MTN MoMo", "Vodafone Cash", "AirtelTigo Money", "Visa", "Mastercard"];

export async function Footer() {
  const year = new Date().getFullYear();
  return (
    <footer id="site-footer" className="mt-16 border-t border-border bg-foreground text-background/80 text-sm">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 pt-12 pb-8">
        ...
        <div className="mt-12 pt-6 border-t border-background/15 flex flex-wrap items-center gap-2">
          <span className="text-[11px] uppercase tracking-[0.14em] text-background/50 mr-1">
            We accept
          </span>
          {PAYMENTS.map((p) => (
            <span
              key={p}
              className="inline-flex items-center px-2.5 py-1 rounded border border-background/20 text-[11px] font-semibold text-background/80"
            >
              {p}
            </span>
          ))}
        ...
        <div className="mt-6 pt-6 border-t border-background/15 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
